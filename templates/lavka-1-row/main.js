...
const syncLayout = (node) => {
  ...
  const primary = node.querySelector('.lavka-one-lt__primary');
  const plate = node.querySelector('.lavka-one-lt__plate');
  if (!primary || !plate) {
    return;
  }
  ...
  const textWidth = measureTextWidth(primary);
  ...
  node.style.setProperty('--lavka-one-plate-width', `${nextWidth}px`);
};
