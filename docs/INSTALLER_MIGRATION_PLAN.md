...
### Код-ревью после Этапов 1–5 (high recall)
Две находки, обе исправлены/учтены:
1. **КРИТ (исправлено):** `auto-updater.cjs` перед `quitAndInstall()` вызывал
   тяжёлый `requestQuitForUpdate()` (уничтожал окна + `app.quit()` +
   `app.exit(0)` через 1.5 с) → приложение жёстко выходило, гонясь с NSIS-
   установщиком, апдейт не применялся. Заменено на лёгкий `authorizeCloseForUpdate`
   (только авторизация закрытия + снятие close-guard); гашение теперь целиком на
   `quitAndInstall`. Полный цикл проверяется на первом nsis-релизе.
2. **reset/uninstall под nsis (исправлено):** `launchCleanupAndQuit` был завязан на
   `PORTABLE_EXECUTABLE_*` (под nsis отсутствуют). **Reset** → fallback на
   `process.execPath`. **Remove completely** → cleanup-скрипт находит
   `Uninstall *.exe` в install-dir и запускает его тихо (`/S`), затем стирает
   userData (fallback на удаление exe для portable/dev). Юнит-тест добавлен
   (155/155). Проверяется на живой установке при cutover.
