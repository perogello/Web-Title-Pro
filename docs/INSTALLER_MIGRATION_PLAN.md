...
2. **reset/uninstall под nsis (частично):** `launchCleanupAndQuit` был завязан на
   `PORTABLE_EXECUTABLE_*` (под nsis отсутствуют) → **Reset исправлен** через
   fallback на `process.execPath`. «Remove completely» пока best-effort (удаляет
   только exe). **Follow-up:** запускать NSIS-uninstaller (очистка ярлыков/реестра).
